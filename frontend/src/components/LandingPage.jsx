...
export default function LandingPage({ onGetStarted }) {
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      {/* ── Nav ── */}
      <nav className="flex items-center justify-between px-8 h-16 border-b border-slate-200 bg-white/80 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          <button onClick={onGetStarted} className="text-sm font-medium hover:text-slate-600 transition-colors text-slate-600">
            Sign in
          </button>
          <button onClick={onGetStarted} className="bg-slate-900 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-90 transition-opacity">
            Get started
          </button>
        </div>
      </nav>
      ...
      <section className="pt-24 pb-16 px-8 max-w-6xl mx-auto text-center">
        ...
        <h1 className="text-5xl md:text-7xl font-bold tracking-tight mb-6 text-slate-900 text-balance leading-[1.05]">
        ...
        <div className="flex flex-wrap justify-center gap-3">
          ...
          <a
            href="#workspace"
            className="border border-slate-200 text-slate-700 px-7 py-3.5 rounded-lg font-semibold text-sm hover:bg-slate-50 transition-colors"
          >
            See the workspace
          </a>
        </div>
          ...
          <main className="flex-1 flex flex-col bg-white">
            <div className="flex-1 p-5 md:p-6 space-y-6">
              <div className="flex gap-4">
                <div className="w-8 h-8 rounded-lg bg-slate-100 border border-slate-200 shrink-0" />
                <div className="bg-slate-100 p-4 rounded-2xl rounded-tl-none text-xs text-slate-800 max-w-[80%] leading-relaxed">
                  What are the mitigation strategies for supply chain risks?
                </div>
              </div>
              <div className="flex gap-4">
                <div className="w-8 h-8 rounded-lg bg-slate-900 shrink-0 flex items-center justify-center text-white text-[10px] font-black">D</div>
                <div className="p-2 text-xs space-y-3 max-w-[85%] leading-relaxed">
      ...
      {/* ── Features ── */}
      <section id="features" className="bg-slate-50/50 py-24 px-8 border-y border-slate-200">
        <div className="max-w-5xl mx-auto">
          <div className="grid md:grid-cols-3 gap-12">
            {[
              ...
              <div key={it.n}>
                <div className="w-10 h-10 bg-white border border-slate-200 rounded-lg mb-6 flex items-center justify-center font-bold text-slate-800 text-sm shadow-sm">
                  {it.n}
                </div>
